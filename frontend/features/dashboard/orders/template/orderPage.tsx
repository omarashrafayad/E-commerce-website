...
export default function OrdersPage() {
    ...
    const handleEditStatus = (order: IOrder) => {
    ...
    const onSubmit = async (data: OrderStatusData) => {
        ...
        try {
            ...
            toast.error(axiosError.response?.data?.message || "Status update failed");
        }
    };

    const COLUMNS = [
        { id: "_id", header: "Order ID", cell: (_: unknown, row: IOrder) => <span className="font-mono font-bold text-primary">{row._id.slice(-8)}</span>, className: "w-24" },
        {
            id: "customer",
            header: "Customer",
            cell: (_: unknown, row: IOrder) => (
                <div className="flex flex-col">
                    <span className="font-bold text-zinc-900 dark:text-zinc-100">{(row.user as { name: string })?.name || "Guest"}</span>
                    <span className="text-xs text-zinc-500 font-medium tracking-tight">{(row.user as { email: string })?.email || "No Email"}</span>
                </div>
            )
        },
        { id: "total", header: "Total Price", cell: (_: unknown, row: IOrder) => <span className="font-black text-zinc-900">${row.totalOrderPrice}</span> },
        {
            id: "status",
            header: "Status",
            cell: (_: unknown, row: IOrder) => {
        ...
        { id: "date", header: "Created At", cell: (_: unknown, row: IOrder) => <span className="text-xs text-zinc-500">{new Date(row.createdAt).toLocaleDateString()}</span> },
        {
            id: "actions",
            header: "Actions",
            cell: (_: unknown, row: IOrder) => (
    ...
    const filteredOrders = orders.filter((o: IOrder) =>
        ...
        ((o.user as { name: string })?.name?.toLowerCase().includes(searchTerm.toLowerCase()))
    );
